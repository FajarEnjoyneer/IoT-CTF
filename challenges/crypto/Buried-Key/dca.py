import struct

sbox = [
    
]

def get_bit(val, bit):
    return (val >> bit) & 1

def dca(table_data):
    # table_data is 256 * 4 bytes
    entries = struct.unpack("<256I", table_data)
    
    best_key = 0
    max_corr = 0
    
    for k in range(256):
        corr_sum = 0
        for target_bit in range(8): # bit of S-box output
            for table_bit in range(32): # bit of table entry
                # Compute correlation between S-box(x ^ k) bit target_bit
                # and entries[x] bit table_bit
                cnt = 0
                for x in range(256):
                    s_val = sbox[x ^ k]
                    t_val = entries[x]
                    if get_bit(s_val, target_bit) == get_bit(t_val, table_bit):
                        cnt += 1
                
                corr = abs(cnt - 128)
                if corr > corr_sum:
                    corr_sum = corr
        
        if corr_sum > max_corr:
            max_corr = corr_sum
            best_key = k
            
    return best_key, max_corr

with open("round1_tboxes.bin", "rb") as f:
    data = f.read()

key = []
for i in range(16):
    table_data = data[i*1024 : (i+1)*1024]
    k, corr = dca(table_data)
    key.append(k)
    print(f"Byte {i}: {k:02x} (corr: {corr})")

print("Full key:", "".join(f"{k:02x}" for k in key))
