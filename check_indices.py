def get_sbox():
    return [
        
    ]

def galois_mul(a, b):
    p = 0
    for i in range(8):
        if b & 1: p ^= a
        hi = a & 0x80
        a <<= 1
        if hi: a ^= 0x1b
        a &= 0xff
        b >>= 1
    return p

def main():
    sbox = get_sbox()
    sbox_inv = [0]*256
    for i, v in enumerate(sbox): sbox_inv[v] = i
    
    with open("challange/crypto/Buried-Key/buried_key", "rb") as f:
        f.seek(0xb6040)
        final_dec_raw = f.read(16 * 256)
    
    final_dec_inv = []
    for i in range(16):
        table = final_dec_raw[i*256 : (i+1)*256]
        inv = [0] * 256
        for x, v in enumerate(table): inv[v] = x
        final_dec_inv.append(inv)
    
    def get_standard(hex_str):
        b = bytes.fromhex(hex_str)
        return bytes([final_dec_inv[i][b[i]] for i in range(16)])

    correct = get_standard("88f98d5e605eac3ee60fb40913896498")
    f1 = get_standard("75f98d5e605eac2de60fc90913756498")
    
    # Try different column mappings
    # Standard: Col 0 -> Row 0, 1, 2, 3
    # ShiftRows: 0->0, 1->13, 2->10, 3->7
    # Wait, my previous indices [0, 13, 10, 7] WERE correct for the output of ShiftRows.
    
    diff_std = [correct[i] ^ f1[i] for i in range(16)]
    indices = []
    for i in range(16):
        if diff_std[i] != 0: indices.append(i)
    print(f"Indices with diff: {indices}")

if __name__ == "__main__":
    main()
