def get_sbox():
    return [
        
    ]

def galois_mul(a, b):
    p = 0
    for i in range(8):
        if b & 1: p ^= a
        hi = a & 0x80
        a <<= 1
        if hi: a ^= 0x1b
        a &= 0xff
        b >>= 1
    return p

def main():
    sbox = get_sbox()
    sbox_inv = [0]*256
    for i, v in enumerate(sbox): sbox_inv[v] = i
    
    with open("challange/crypto/Buried-Key/buried_key", "rb") as f:
        f.seek(0xb6040)
        final_dec_raw = f.read(16 * 256)
    
    final_dec_inv = []
    for i in range(16):
        table = final_dec_raw[i*256 : (i+1)*256]
        inv = [0] * 256
        for x, v in enumerate(table): inv[v] = x
        final_dec_inv.append(inv)
    
    def get_standard(hex_str):
        b = bytes.fromhex(hex_str)
        return bytes([final_dec_inv[i][b[i]] for i in range(16)])

    correct = get_standard("88f98d5e605eac3ee60fb40913896498")
    f1 = get_standard("75f98d5e605eac2de60fc90913756498")
    
    diff_std = [correct[i] ^ f1[i] for i in range(16)]
    indices = [0, 13, 10, 7]
    
    for delta in range(1, 256):
        exp = [galois_mul(2, delta), delta, delta, galois_mul(3, delta)]
        # Check if this delta could be the one
        possible_per_byte = [0]*4
        for i in range(4):
            idx = indices[i]
            diff_out = diff_std[idx]
            for k in range(256):
                x = sbox_inv[correct[idx] ^ k]
                if sbox[x ^ exp[i]] == (f1[idx] ^ k):
                    possible_per_byte[i] += 1
        
        if all(possible_per_byte):
            print(f"Delta {delta} is possible! Counts: {possible_per_byte}")

if __name__ == "__main__":
    main()
